...
function renderListe(f) {
  ...
  cont.innerHTML = list.map(d => {
    ...
    const signé = String(d.signe||'').toLowerCase()==='true' ? `<span style="color:var(--g);font-size:11px;margin-left:8px">${icon('check',12)} Signé</span>` : '';
...
function renderDetail() {
  ...
  document.getElementById('detailCont').innerHTML=`
    <div style="background:white;border-radius:8px;border:1px solid var(--mid);padding:20px 24px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:flex-start;gap:16px">
      <div>
        <div style="font-size:20px;font-weight:700">${d.nom}${String(d.signe||'').toLowerCase()==='true'?`<span style="background:var(--g);color:white;font-size:11px;padding:2px 8px;border-radius:10px;margin-left:8px">${icon('check',11)} Signé</span>`:''}</div>
      ...
      <div style="text-align:right">
        ...
        <div style="font-size:12px;margin-top:6px;color:${String(d.signe||'').toLowerCase()==='true'?'var(--gd)':'#e65100'}">${String(d.signe||'').toLowerCase()==='true'?icon('check',12)+' Signé le '+d.sig_date:icon('clock',12)+' En attente'}</div>
      </div>
    </div>
